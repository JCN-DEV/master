        ...
        angular.module('stepApp')
            .controller('ProfessorListController',
             ['$scope', 'entity', '$state', '$modal', 'TimeScaleApplication', 'TimescaleApplicationList', 'MpoApplication', 'Employee', 'Institute' ,'EmployeeSearch', 'EmployeeInstitute', 'ParseLinks', 'MpoApplicationLogEmployeeCode' , 'MpoApplicationInstitute','MpoApplicationList','Principal','InstLevelByName','PrincipleApplicationSummaryList','PrincipleApplicationsByLevel','ProfessorApplication',
             function ($scope, entity, $state, $modal, TimeScaleApplication, TimescaleApplicationList, MpoApplication, Employee, Institute ,EmployeeSearch, EmployeeInstitute, ParseLinks, MpoApplicationLogEmployeeCode , MpoApplicationInstitute,MpoApplicationList,Principal,InstLevelByName,PrincipleApplicationSummaryList,PrincipleApplicationsByLevel,ProfessorApplication) {
                ...
                TimeScaleApplication.query({page: $scope.page, size: 100}, function(result, headers) {
                 ...
                 if(entity.pending === "pending" && (Principal.hasAnyAuthority(['ROLE_DIRECTOR']) || Principal.hasAnyAuthority(['ROLE_DG']))){
                     PrincipleApplicationSummaryList.query({}, function(result){
                 ...
                 $scope.ssc = function () {
                     InstLevelByName.get({name: "SSC (VOC)"}, function(result){
                         PrincipleApplicationsByLevel.query({status: 0, levelId: result.id}, function(result2){
                 ...
                 $scope.forwardSummaryList = function () {
                     angular.forEach($scope.summaryList, function (data) {
                         ProfessorApplication.get({id: data.MPO_ID}, function(result){
                             result.adForwarded = true;
                             console.log(result);
                             ProfessorApplication.update(result);
                         });
                         // location.reload();
                     });
