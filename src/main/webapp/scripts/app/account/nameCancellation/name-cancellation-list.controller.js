        ...
        angular.module('stepApp')
            ...
            ['$scope', 'entity', '$state', '$modal','NameCnclApplicationList','NameCnclApplication', 'TimeScaleApplication', 'TimescaleApplicationList', 'MpoApplication', 'Employee', 'Institute' ,'EmployeeSearch', 'EmployeeInstitute', 'ParseLinks', 'MpoApplicationLogEmployeeCode' , 'MpoApplicationInstitute','MpoApplicationList',
            function ($scope, entity, $state, $modal,NameCnclApplicationList,NameCnclApplication, TimeScaleApplication, TimescaleApplicationList, MpoApplication, Employee, Institute ,EmployeeSearch, EmployeeInstitute, ParseLinks, MpoApplicationLogEmployeeCode , MpoApplicationInstitute,MpoApplicationList) {

                $scope.timescaleAppliedLists = entity;

                $scope.employees = [];
                $scope.page = 0;
                NameCnclApplicationList.query({status:1,page: $scope.page, size: 100}, function(result, headers) {
                    // $scope.pendingApplications = result;
                    console.log(result);
                });
                NameCnclApplicationList.query({status:0,page: $scope.page, size: 100}, function(result, headers) {
                    // $scope.pendingApplications = result;
                    console.log(result);
                });
                NameCnclApplication.query({page: $scope.page, size: 100}, function(result, headers) {
