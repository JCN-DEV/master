...
angular.module('stepApp')
    ...
    ['$scope', '$rootScope', '$state','InfoCorrectLogsByEmpCode', '$stateParams', 'Sessions', 'Training', 'InstEmployeeCode','MpoApplicationLogEmployeeCode','MpoApplication', 'PayScale', 'Jobapplication', 'Principal', 'ParseLinks', 'ApplicantEducation', 'Institute', 'User', 'Upazila', 'Course', 'Employee', 'DataUtils', 'StaffCount',
    function ($scope, $rootScope, $state,InfoCorrectLogsByEmpCode, $stateParams, Sessions, Training, InstEmployeeCode,MpoApplicationLogEmployeeCode,MpoApplication, PayScale, Jobapplication, Principal, ParseLinks, ApplicantEducation, Institute, User, Upazila, Course, Employee, DataUtils, StaffCount) {
        ...
        if(Principal.hasAnyAuthority(['ROLE_INSTEMP'])){
            Principal.identity().then(function (account) {
                ...
                InstEmployeeCode.get({'code':$scope.account.login},function(res){
                    ...
                    InfoCorrectLogsByEmpCode.get({'code':$scope.employee.code}, function(result) {
                        console.log('>>>>>>>>>>>>>>');
                        console.log(result);
