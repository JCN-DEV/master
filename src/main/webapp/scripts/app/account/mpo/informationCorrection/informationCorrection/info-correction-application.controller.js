angular.module('stepApp').controller('InfoCorrectionAppController',
    ...
    function ($scope, $stateParams,$rootScope, $parse, $state, entity, Auth,InstEmplTraining,BEdApplication, BEdApplicationCheck, TimeScaleApplicationCheck, Principal,TimeScaleApplication, $rootScope, CurrentInstEmployee, Employee, User, Institute, ApplicantEducation, Training, AttachmentCategoryModule, Attachment, MpoApplication, InstEmployeeCode, InstEmployeeAddressInstitute, AttachmentEmployee, MpoApplicationCheck,AttachmentCategoryByApplicationName,InformationCorrectionByEmployeeCode,InformationCorrection,ActiveInstEmplDesignation) {
        ...
        $scope.instEmplDesignations = ActiveInstEmplDesignation.query();
        Principal.identity().then(function (account) {
            InformationCorrectionByEmployeeCode.get({'code':account.login},function(res){
                console.log(res);
                if(res.id == 0){
        ...
        AttachmentCategoryByApplicationName.get({name: 'InfoCorrect'}, function (result) {
            $scope.attachmentCategoryList = result;
        });
        ...
        $scope.save = function () {
           /* TimeScaleApplicationCheck.get({'code': $scope.account.login}, function (res) {
            ...
            $scope.informationCorrection.instEmployee = $scope.employee;
            InformationCorrection.save($scope.informationCorrection);

            console.log($scope.informationCorrection);
            //employee attachment
            ...
            $state.go('infoCorrectAppStatus',{},{reload:true});
            /*$state.go('mpo.employeeTrack', {}, {reload: true});*/
        };
