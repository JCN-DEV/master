...
import gov.step.app.web.rest.dto.ManagedUserDTO;
...
UserResource {
    ...
    @Timed
    @Transactional
    @Secured({AuthoritiesConstants.ADMIN,AuthoritiesConstants.MPOADMIN,AuthoritiesConstants.INSTITUTE})
    ...
    //method created by galeb
    //authorities does not set here which is in updateUser().
    /**
     * PUT  /users -> Updates an existing User.
     */
    @RequestMapping(value = "/users/customUpdate",
        method = RequestMethod.PUT,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    @Transactional
    @Secured({AuthoritiesConstants.ADMIN,AuthoritiesConstants.MPOADMIN,AuthoritiesConstants.INSTITUTE})
    public ResponseEntity<ManagedUserDTO> updateUserCustom(@RequestBody ManagedUserDTO managedUserDTO) throws URISyntaxException {
        log.debug("REST request to update User : {}", managedUserDTO);
        return userRepository
            .findOneById(managedUserDTO.getId())
            .map(user -> {
    ...
    /**
     * GET  /blockusers -> get all users.
     */
    @RequestMapping(value = "/blockusers",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    @Transactional(readOnly = true)
    public ResponseEntity<List<ManagedUserDTO>> getAllBlockedUsers(Pageable pageable)
        throws URISyntaxException {

        Page<User> page = userRepository.findAllBlockedUser(pageable, false);
        List<ManagedUserDTO> managedUserDTOs = page.getContent().stream()
            .map(user -> new ManagedUserDTO(user))
            .collect(Collectors.toList());
    ...
    /**
     * GET  /blockusers/:login -> get the "login" user.
     */
    @RequestMapping(value = "/blockusers/{login}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<ManagedUserDTO> getBlockedUser(@PathVariable String login) {
        log.debug("REST request to get User : {}", login);
        System.out.println("\n REST request to get User >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> "+login);
        return userService.getUserWithAuthoritiesByLogin(login)
            .map(ManagedUserDTO::new)
            .map(managedUserDTO -> new ResponseEntity<>(managedUserDTO, HttpStatus.OK))
            .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
