...
import gov.step.app.repository.AuthorRepository;
import gov.step.app.repository.search.AuthorSearchRepository;
...
public class AuthorResource {
    ...
    public ResponseEntity<Author> updateAuthor(@Valid @RequestBody Author author) throws URISyntaxException {
        ...
        return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert("author", author.getId().toString()))
            .body(result);
    }
    ...
    @Timed
    public ResponseEntity<List<Author>> getAllAuthors(Pageable pageable) throws URISyntaxException {
    ...
    /**
     * SEARCH /_search/authors/:query -> search for the author corresponding to
     * the query.
     */
    @RequestMapping(value = "/_search/authors/{query}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public List<Author> searchAuthors(@PathVariable String query) {
        return StreamSupport.stream(authorSearchRepository.search(queryStringQuery(query)).spliterator(), false)
